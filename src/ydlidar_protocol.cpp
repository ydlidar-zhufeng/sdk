...
result_t read_command(Serial *serial, uint8_t *buffer, size_t size,
  ...
  if (!IS_OK(ans)) {
      LOG_ERROR("wait_for_data:%s",(ans == -1)?"RESULT_TIMEOUT":"RESULT_FAIL");
    error = ReadError;
    return ans;
  }
  ...
  if (r < size) {
      LOG_ERROR("read_data:%s",(ans == -1)?"RESULT_TIMEOUT":"RESULT_FAIL");
    error = ReadError;
    ans = RESULT_TIMEOUT;
  }
      ...
      if (header.packageHeaderLSB != HEADER_LSB) {
        ...
        if (IS_OK(check_package_header_t(header, error))) {
          ...
          break;
            ...
            LOG_ERROR("check_package_header_t error ,error:%d",error);
          p2header[0] = 0x00;
          ans = RESULT_TIMEOUT;
        }
...
result_t read_response_scan_intensity_t(Serial *serial,
  ...
  if (IS_OK(ans)) {
    ...
    if (IS_OK(ans)) {
      if (checksum_response_scan_intensity_packet_t(scan) != scan.header.checkSum) {
          LOG_ERROR("read_data_timeout:%s",(ans == -1)?"RESULT_TIMEOUT":"RESULT_FAIL");
        ans = RESULT_TIMEOUT;
        error = CheckSumError;
      }
    }
  ...
  if (!IS_OK(ans)) {
      LOG_ERROR("read_response_scan_header_t:%s",(ans == -1)?"RESULT_TIMEOUT":"RESULT_FAIL");
    reset_ct_packet_t(ct);
  }
